...
module Leve
  # An agent is a directory (eve's model). Launch leve in it and it picks up
  ...
  #   .leve/sessions/   where the durable session logs live
  #
  # Nothing here is required: with no files at all leve is a plain coding
  # agent in the current directory, which is why `leve` in a random checkout
  # still works.
  class Project
    ...
    # the character.
    EXTRA_PROMPT_FILES = %w[SOUL.md agent/SOUL.md .leve/SOUL.md].freeze
    AGENT_FILES = %w[agent.rb .leve/agent.rb].freeze
    ...
    # What makes a directory an agent directory. leve refuses to launch
    ...
    #
    # A plain checkout (leve --plain) has no workspace; the checkout is the
    # work.
    ...
    def sessions_dir = File.join(@root, ".leve", "sessions")
    ...
    def conversation_path(name = "main", fresh: false)
      ...
      index_path = File.join(@root, ".leve", "conversations.json")
      ...
      raise ArgumentError, "invalid .leve/conversations.json: #{e.message}"
    end
    ...
    def load_sandbox
      ...
      # compatibility fallback for agents generated before Leve 0.8.
    ...
    SCAFFOLD = {
      ...
      "channels/tui.rb" => <<~'RUBY',
        # Leve's default channel: a small visitor that delegates to the library's
      ...
      "workspace/skills/heartbeat/SKILL.md" => <<~'MD',
        ...
        Edit `/workspace/HEARTBEAT.yml`. Leve watches the complete file and applies each
        ...
        Leve appends the strict response contract to every task. Return exactly one of:

        - `SILENCE`
        - `Message: one paragraph for the user`
        - `Steer: instruction for the main conversation`

        Anything else is a durable heartbeat error. Before each task Leve refreshes
      ...
      "sandbox.rb" => <<~'RUBY',
        ...
        #
        # Microsandbox is mandatory. Leve binds directly to the microsandbox Rust
        # crate through its in-repo native extension and refuses to run without it;
        # there is no host/local mode.
        #
      ...
      "workspace/AGENTS.md" => <<~'MD',
        ...
        - Reusable procedures belong in `skills/<name>/SKILL.md`; Leve reloads them live.

        ## Dreaming
        `DREAM.md` consolidates conversation and note noise into current knowledge. Leve
      ...
      "workspace/DREAM.md" => <<~'MD',
        ...
        project code, tests, configuration, or Leve's durable session files. Record a short
    ...
    # are candidates, not assumptions that Leve owns the user's edits.
    def self.init(root = Dir.pwd, name: nil, force: false, update: false)
      ...
      FileUtils.mkdir_p(File.join(root, ".leve", "sessions"))
      gitignore = File.join(root, ".gitignore")
      existing = File.file?(gitignore) ? File.read(gitignore) : ""
      missing_ignores = [".leve/", "workspace/RECENT_CONVERSATIONS.md"].reject do |line|
