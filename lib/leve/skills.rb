...
module Leve
  ...
  #
  # Discovery is strictly inside the agent directory: stable agent-owned skills
  # live in workspace/skills/, where the agent can inspect and edit them through
  # the VM. Nothing is loaded from the agent root or $HOME.
  #
  ...
  module Skills
    ...
    PROJECT_DIRS = ["workspace/skills"].freeze
    ...
    def load(cwd: Dir.pwd, extra_dirs: [], user: true)
      ...
      local = roots(root, user: false) + extra_dirs.map { File.expand_path(_1, root) }
      dirs = local.select { |dir| dir == root || dir.start_with?("#{root}/") }
      skills = {}
      diagnostics = []
      real_paths = {}
      dirs.each do |dir|
    ...
    def workspace_path(root) = File.join(File.expand_path(root), WORKSPACE_DIR)

    def workspace_skill?(skill, root)
      path = File.expand_path(skill["path"])
      dir = workspace_path(root)
      path == dir || path.start_with?("#{dir}/")
    end

    # Fingerprint every file, not only SKILL.md: a skill may reference scripts,
    # templates, or examples beside it, and changing those must refresh what the
    # model is told is available.
    def workspace_fingerprint(root)
      dir = workspace_path(root)
      return Digest::SHA256.hexdigest("") unless File.directory?(dir)

      digest = Digest::SHA256.new
      Dir.glob(File.join(dir, "**", "*"), File::FNM_DOTMATCH).sort.each do |path|
        next unless File.file?(path)

        digest << path.delete_prefix("#{dir}/") << "\0" << File.binread(path) << "\0"
    ...
    # A tail message used when mutable workspace skills change. Keeping this out
    # of the system prompt preserves the provider's cached prefix.
    def update_message(skills)
      section = format_for_prompt(skills)
      return "<available_skills_update>\nNo workspace skills are currently available.\n</available_skills_update>" if section.empty?

      "<available_skills_update>\nWorkspace skills changed. Use this current catalog for subsequent work.\n" \
        "#{section}\n</available_skills_update>"
    end
