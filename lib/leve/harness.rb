...
module Leve
  ...
  class Harness
    HOOKS = %w[before_run before_resume before_run_end transform_context before_request
               after_response before_tool after_tool before_compaction].freeze
    ...
    def start_heartbeat
      ...
      @heartbeat = Heartbeat::Runner.new(
        ...
        state_path: File.join(@project.root, ".leve", "heartbeat.json")
      ).start
    end
    ...
    def quietly
      yield
    rescue Ractor::ClosedError, Ractor::Error, Leve::RemoteError
      nil
    end
    ...
    class LaneHandle
      ...
      def skill(name, additional_instructions = nil)
        ...
        prompt(Leve::Skills.invocation_message(sk, additional_instructions))
      end
      ...
      def close
        call("close")
      rescue Leve::RemoteError, Ractor::ClosedError
        nil
      end
