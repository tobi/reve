  ...
  module Lane
    ...
    def self.spawn(store:, hub:, host:, lane:, config:)
      Ractor.new(store, hub, host, lane, IPC.encode(config), name: "lane:#{lane}") do |st, hb, ho, ln, cfg_json|
        runner = Leve::Lane::Runner.new(store: st, hub: hb, host: ho, lane: ln,
    ...
    class Runner
      ...
      def initialize(store:, hub:, host:, lane:, config:)
        ...
        @lane = lane
        @config = config
        @session = Leve::Session.new(store, lane: lane)
        @mutex = Mutex.new
      ...
      def control_loop
        until @closed
          msg = Ractor.receive
          Leve::IPC.serve(msg) { |op, arg, port| handle(op, arg, port) }
        end
      end
      ...
      def accept_run(arg, port)
        ...
        prompt_content = hook["prompt"] if hook && hook["prompt"]
      ...
      def run_in_worker(port, &blk)
        ...
        @worker = Thread.new do
          ...
          Leve::IPC.reply(port, result) if port
        end
        Leve::IPC::DEFER
      end
      ...
      def run_tool_batch(assistant)
        ...
        AgentLoop.execute_tool_batch(assistant, tool_declarations, callbacks,
                                     { cwd: @config["cwd"] || Dir.pwd,
                                       ...
                                       cancel_remote: method(:cancel_host_tool).to_proc },
                                     method(:emit_raw).to_proc, -> { @abort })
      end

      # A project tool: the host owns the block (and the sandbox connection).
      def run_host_tool(name, args, call_id = nil)
        Leve::IPC.call(@host, "run_tool", { "tool" => name, "args" => args, "lane" => @lane,
                                            "callId" => call_id })
      rescue Leve::RemoteError => e
        { "content" => [{ "type" => "text", "text" => "tool #{name} failed: #{e.message}" }], "isError" => true }
      end

      def cancel_host_tool(call_id)
        Leve::IPC.cast(@host, "cancel_tool", { "callId" => call_id, "lane" => @lane })
      end
      ...
      def tool_declarations
        map = {}
        Tools.declarations(active_tool_names & Tools.names).each do |declaration|
          runner = Tools.sandboxed?(declaration["name"]) ? "host" : "ractor"
          map[declaration["name"]] = declaration.merge("runner" => runner)
        end
      ...
      # The base prompt is configuration; resolved per request, it survives
      # compaction with the branch.
      def system_prompt
        @config["systemPrompt"].to_s
      end
      ...
      def check_cache_prefix(system, messages, tools)
        ...
        # Some breaks are the price of something the user asked for: a model
        # switch, a compaction. Those are announced, not alarming.
        deliberate = @cache_break
        @cache_break = nil
        @cache_usage_exempt = deliberate == "compaction"
      ...
      def record_cache_usage(usage)
        ...
        return if @cache_usage_exempt || input <= 0

        miss_rate = (input - read).clamp(0, input).to_f / input
        return unless miss_rate > 0.30

        @cache_stats["misses"] += 1
        percent = (miss_rate * 100).round
        reason = "provider reported #{percent}% uncached input (#{input - read}/#{input} tokens)"
        emit("cache_miss", { "reason" => reason, "input" => input, "cacheRead" => read,
                             "missRate" => miss_rate })
        warn_loudly("PROMPT CACHE MISS", [reason])
      ensure
        @cache_usage_exempt = false
      end
      ...
      def hook_call(name, payload)
        Leve::IPC.call(@host, "hook", { "hook" => name, "lane" => @lane, "payload" => payload })
      rescue Leve::RemoteError => e
