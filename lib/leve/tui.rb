...
module Leve
  ...
  class InteractiveAgentTUI
    ...
    COMMANDS = %w[help exit quit abort resume compact new verbose skill model models think
                  tools state cache sandbox output agents lanes lane log steer next].freeze
    ...
    def render(ev)
      return if ev["lane"] && ev["lane"] != @lane &&
                (@background_lanes[ev["lane"]] || @h.background_lane?(ev["lane"]))

      tag = ev["lane"] == @lane ? "" : s(:magenta, "[#{ev["lane"]}] ")
      case ev["type"]
      when "run_start", "run_resume", "compaction_start"
      ...
      when "cache_miss"
        emit("\e[43;30;1m CACHE MISS \e[0m " + s(:yellow, ev["reason"].to_s))
      when "heartbeat_task_start"
        @background_lanes[ev["laneName"]] = true
        emit(s(:dim, "  ♥ heartbeat #{ev["task"]} started"))
      when "heartbeat_message"
        emit(s(:magenta, "  ♥ #{ev["task"]}: ") + ev["message"].to_s)
      when "heartbeat_steer"
        emit(s(:magenta, "  ♥ #{ev["task"]} steered main: ") + ev["command"].to_s)
      when "heartbeat_reloaded"
        emit(s(:dim, "  ♥ heartbeat reloaded: #{(ev["tasks"] || []).join(", ")}"))
      when "heartbeat_error"
        emit(s(:red, "  ♥ heartbeat #{ev["task"]}: #{ev["message"]}"))
    ...
    def status_line
      ...
      s(:bold, "leve") + s(:dim, "  #{bits.join(" · ")}")
    end

    def banner
      st = @h.state
      emit(status_line)
      emit(s(:dim, "  conversation #{@h.conversation_name} · cwd #{Dir.pwd}"))
      if @h.project&.agent?
    ...
    def argument_candidates(cmd)
      ...
      when "tools" then Leve::Tools.names
    ...
    # `!command`: the user's own shell command, executed in the same mandatory
    # microVM as model bash. It lands in the conversation as a bash_execution
    # entry — durable like any other write, deferred when a step is in flight.
    def shell(command)
      return emit(s(:dim, "  usage: !<command>")) if command.empty?

      emit(s(:blue, "$ ") + command + s(:dim, "  (sandbox)"))
      @shell_running = command.split.first
      @shell_cancel = false
      Thread.new do
        started = Time.now
        r = @h.sandbox.exec(command, timeout: 600, cancel: -> { @shell_cancel })
        code = r["exitCode"]
        output = "#{r["stdout"]}#{r["stderr"]}"
        all = output.lines.map(&:rstrip)
        limit = @verbose ? 2000 : 40
        printed = 0
        all.each do |line|
          printed += 1
          emit(s(:gray, "  #{clip(line, width - 4)}")) if printed <= limit
        end
        ...
        res = lane_handle.append_bash(command, Leve::Tools.overspill(output, "bash").first, code)
    ...
    def start_run(&blk)
      ...
      @run_thread = Thread.new do
        blk.call
      rescue Leve::RemoteError, Ractor::ClosedError => e
    ...
    def handle_interrupt
      ...
      when :force_quit
        emit(s(:yellow, "^C") + s(:dim, "  quitting; the operation stays resumable (leve -r)"))
        Term.restore!
        exit!(130)
      when :clear_line
        @line.reset
    ...
    def handle_eof
      ...
      when :hint_leave
        ...
        :continue
      when :leave_running
        emit(s(:dim, "  leaving the run open; resume it with leve -r"))
        @leave_running = true
        :exit
      else
        :exit
    ...
    def cmd_new
      return emit(s(:yellow, "  finish or abort the current operation before /new")) if busy?

      old = @h
      replacement, suspended = old.new_session
      had_renderer = @renderer&.alive?
      old.close(close_sandbox: false)
      @renderer&.join(1)
      @h = replacement
      @suspended = suspended
      @lane = "main"
      ...
      @usage = { "input" => 0, "output" => 0 }
      start_renderer if had_renderer
      emit(s(:green, "  new session"), s(:dim, "  #{@h.session_path || "memory"}"))
    rescue StandardError => e
      emit(s(:red, "  could not create session: #{e.message}"))
    end
    ...
    def cmd_tools(rest)
      if rest && !rest.empty?
        ...
        builtin = Leve::Tools.declarations.map { _1.merge("where" => "ractor") }
