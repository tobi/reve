  ...
  module Observer
    ...
    def spawn(store:)
      Ractor.new(store, name: "observer") do |st|
        ...
        until closed
          msg = Ractor.receive
          Leve::IPC.serve(msg) do |op, arg, port|
            case op
            when "emit"
              apply.call(arg)
              json = Leve::IPC.encode(arg)
              dead = []
              watchers.each do |w|
            ...
            when "watch"
              ...
              payload = arg["lane"] ? snapshot.call(arg["lane"]) : Leve::Observer.session_snapshot(session, mirrors)
              Leve::IPC.reply(port, payload)
              Leve::IPC::DEFER
            when "unwatch"
