  ...
  class InteractiveAgentTUI
    ...
    def shell(command)
      ...
      Thread.new do
        ...
        res = lane_handle.append_bash(command, Reve::Tools.overspill(output, "bash").first, code)
    ...
    def start_run(&blk)
      ...
      @run_thread = Thread.new do
        blk.call
      rescue Reve::RemoteError, Ractor::ClosedError => e
    ...
    def handle_interrupt
      ...
      when :force_quit
        emit(s(:yellow, "^C") + s(:dim, "  quitting; the operation stays resumable (reve -r)"))
        Term.restore!
        exit!(130)
      when :clear_line
        @line.reset
    ...
    def handle_eof
      ...
      when :hint_leave
        ...
        :continue
      when :leave_running
        emit(s(:dim, "  leaving the run open; resume it with reve -r"))
        @leave_running = true
        :exit
      else
        :exit
    ...
    def cmd_tools(rest)
      if rest && !rest.empty?
        ...
        builtin = Reve::Tools.declarations.map { _1.merge("where" => "ractor") }
