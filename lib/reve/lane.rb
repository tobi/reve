  ...
  module Lane
    ...
    def self.spawn(store:, hub:, host:, lane:, config:)
      Ractor.new(store, hub, host, lane, IPC.encode(config), name: "lane:#{lane}") do |st, hb, ho, ln, cfg_json|
        runner = Reve::Lane::Runner.new(store: st, hub: hb, host: ho, lane: ln,
    ...
    class Runner
      ...
      def initialize(store:, hub:, host:, lane:, config:)
        ...
        @lane = lane
        @config = config
        @session = Reve::Session.new(store, lane: lane)
        @mutex = Mutex.new
      ...
      def control_loop
        until @closed
          msg = Ractor.receive
          Reve::IPC.serve(msg) { |op, arg, port| handle(op, arg, port) }
        end
      end
      ...
      def run_in_worker(port, &blk)
        ...
        @worker = Thread.new do
          ...
          Reve::IPC.reply(port, result) if port
        end
        Reve::IPC::DEFER
      end
      ...
      def run_host_tool(name, args, call_id = nil)
        Reve::IPC.call(@host, "run_tool", { "tool" => name, "args" => args, "lane" => @lane,
                                            "callId" => call_id })
      rescue Reve::RemoteError => e
        { "content" => [{ "type" => "text", "text" => "tool #{name} failed: #{e.message}" }], "isError" => true }
      end

      def cancel_host_tool(call_id)
        Reve::IPC.cast(@host, "cancel_tool", { "callId" => call_id, "lane" => @lane })
      end
      ...
      def hook_call(name, payload)
        Reve::IPC.call(@host, "hook", { "hook" => name, "lane" => @lane, "payload" => payload })
      rescue Reve::RemoteError => e
