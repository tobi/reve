  ...
  module Observer
    ...
    def spawn(store:)
      Ractor.new(store, name: "observer") do |st|
        ...
        until closed
          msg = Ractor.receive
          Reve::IPC.serve(msg) do |op, arg, port|
            case op
            when "emit"
              apply.call(arg)
              json = Reve::IPC.encode(arg)
              dead = []
              watchers.each do |w|
            ...
            when "watch"
              ...
              payload = arg["lane"] ? snapshot.call(arg["lane"]) : Reve::Observer.session_snapshot(session, mirrors)
              Reve::IPC.reply(port, payload)
              Reve::IPC::DEFER
            when "unwatch"
