  ...
  module Tools
    ...
    def spawn(name, args, cwd)
      Ractor.new(name, IPC.encode(args), cwd) do |n, args_json, dir|
        ...
        begin
          result = Reve::Tools.invoke(n, JSON.parse(args_json), dir, -> { cancelled[0] })
        ensure
          watcher.kill
        end
        Reve::IPC.encode(result)
      end
    end
    ...
    def overspill(text, kind, root: Dir.pwd)
      ...
      log_dir = File.join(File.expand_path(root), ".reve", "logs"); FileUtils.mkdir_p(log_dir)
