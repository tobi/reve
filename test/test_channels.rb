...
group "channels are one-file registrations with prompt guidance" do
  registration = Leve::Channels.registrations.find { _1.name == "telegram" }
  eq "the example registers itself", Leve::Channels::Telegram, registration.adapter
  ...
  Dir.mktmpdir do |dir|
    h, = test_harness(cwd: dir, project: false,
                      channel_instructions: Leve::Channels.system_prompts)
...
group "channel KV is durable, namespaced, and private" do
  Dir.mktmpdir do |dir|
    first = Leve::Channels::KV.new(dir, "telegram")
    other = Leve::Channels::KV.new(dir, "other")
...
group "Telegram rich output uses a monotonic state machine" do
  ...
  machine = Leve::Channels::Telegram::RichMessageStateMachine.new(
  ...
  eq "draft starts with a private thinking placeholder", Leve::Channels::Telegram::THINKING,
...
group "Telegram permanently locks input and output to the first private sender" do
  Dir.mktmpdir do |dir|
    kv = Leve::Channels::KV.new(dir, "telegram-pairing-test")
    harness = Struct.new(:main).new(Object.new)
    context = Struct.new(:kv, :harness) do
      def command(*) = nil
    end.new(kv, harness)
    telegram = Leve::Channels::Telegram.new(context)
    sends = []
...
group "channel commands parse JSON objects" do
  ...
  runtime = Leve::Channels::Runtime.new(harness, project)
  ...
  context = Leve::Channels::Context.new(runtime, Leve::Channels.registrations.first)
...
group "channel commands cannot shadow a harness command" do
  ...
  reserved = Leve::InteractiveAgentTUI::RESERVED
  runtime = Leve::Channels::Runtime.new(harness, project, reserved: reserved)
