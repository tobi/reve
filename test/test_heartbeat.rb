...
group "HEARTBEAT.yml is strict and VM-only" do
  Dir.mktmpdir do |dir|
    ...
    task = Reve::Heartbeat.load(path).first
...
group "heartbeat config reload keeps the last valid revision" do
  Dir.mktmpdir do |dir|
    ...
    runner = Reve::Heartbeat::Runner.new(h, workspace: dir, config_path: path,
                                           tasks: Reve::Heartbeat.load(path), state_path: state)
...
group "nonzero VM preparation skips the model turn" do
  Dir.mktmpdir do |dir|
    ...
    vm.define_singleton_method(:exec) do |_command, **_options|
      { "stdout" => "", "stderr" => "not ready", "exitCode" => 7, "cancelled" => false }
    end
