...
group "cut point: keep the recent suffix, summarize the head" do
  ...
  prep = Durable::Compaction.prepare(entries, { "keepRecentTokens" => 200 })
...
group "split turn: one turn too big to keep whole" do
  ...
  prep = Durable::Compaction.prepare(entries, { "keepRecentTokens" => 100 })
...
group "file operations are extracted exactly, not summarized" do
  ...
  prep = Durable::Compaction.prepare(entries, { "keepRecentTokens" => 50 })
  ops = prep["fileOps"]
  ...
  eq "and they are rendered for the summary", true,
     Durable::Compaction.format_file_operations(ops).include?("<modified-files>")
end
...
group "a second compaction updates the previous summary" do
  ...
  prep = Durable::Compaction.prepare(entries, { "keepRecentTokens" => 60 })
  eq "previous summary carried in", "OLD SUMMARY", prep["previousSummary"]
  msg = Durable::Compaction.request_message(prep["messagesToSummarize"],
  ...
  eq "nothing to compact right after a compaction", nil,
     Durable::Compaction.prepare([entries.first], {})
end

group "serialization stops the model from continuing the chat" do
  text = Durable::Compaction.serialize([
...
Dir.mktmpdir do |dir|
  group "compaction end to end: context is summary + kept suffix" do
    ...
    eq "the next request sees the summary", true,
       Durable::Context.messages(h.session.context_entries).first["content"][0]["text"].include?("STRUCTURED SUMMARY")
    h.close
  end
